...
def clean_cms_data(csv_input, csv_output, headers_min):
    ...
    with open(csv_input_file) as fin:    
        ...
        for row in csvin:
            ...
            provider_covid_occuppied_beds  = row['Total Number of Occupied Beds']
            ...
            provider_covid_staff_deaths_total  = row['Staff Total COVID-19 Deaths']
            row_row = [provider_name, provider_fed_id, provider_address, provider_city, provider_county, provider_covid_residents_confirmed, provider_covid_residents_suspected, provider_covid_residents_deaths_total, provider_covid_residents_deaths_covid, provider_covid_occuppied_beds, provider_covid_staff_confirmed_total, provider_covid_staff_suspected_total, provider_covid_staff_deaths_total]
            csv_rows.append(row_row)
...
for state in cms_states:
    state_cms = csv_input_a + '-' + state + csv_input_b + '-min'
